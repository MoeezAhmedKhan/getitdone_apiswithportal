...
if($_POST['token'] == 'as23rlkjadsnlkcj23qkjnfsDKJcnzdfb3353ads54vd3favaeveavgbqaerbVEWDSC')
{
   ...
   if($rider_status == 'idle'){
    $sqlfortask = "SELECT `id`, `user_id`, `dropoff_location`, `latitude`, `longitude`,`status`, `created_at` FROM `task` WHERE `status` != 'completed' AND id NOT IN (SELECT task_id from `task_log`)";
    ...
    if($rowfortask > 0)
    {
        ...
        while($ar1 = mysqli_fetch_array($runfortask))
        {
            ...
            $sqlfortaskdetail = "SELECT `id`, `task_id`, `title`, `name`, `description`, `estimated_cost`, `additional_cost` , `pickup_location`,`latitude`, `longitude`, `menu_item_id`,`status` FROM `task_details` where task_id =".$ar1['id'];
            ...
            if($rowfortaskdetail > 0)
            {
                ...
                while($ar2 = mysqli_fetch_array($runfortaskdetail))
                {
                    ...
                    [
                        ...
                        "additional_cost"=>$ar2['additional_cost'],
                        "pickup_location" => $ar2['pickup_location'],
                        "pickup_latitude" => $ar2['latitude'],
                        "pickup_longitude" => $ar2['longitude'],
            ...
            [
                ...
                "dropoff_latitude" => $ar1['latitude'],
                "dropoff_longitude" => $ar1['longitude'],
                "status "=> $ar1['status'],
                "task_details"=>$myarr,
            ];
            $myarr = [];
