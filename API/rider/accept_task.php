...
if($_POST['token'] == 'as23rlkjadsnlkcj23qkjnfsDKJcnzdfb3353ads54vd3favaeveavgbqaerbVEWDSC')
{
    ...
    $task_details = "SELECT `estimated_cost` , `status` FROM `task_details` WHERE task_id = $task_id";
    $run_task_details = mysqli_query($conn,$task_details);
    $task_details_row = mysqli_num_rows($run_task_details);
    if($task_details_row > 0)
    {  
        $Data = mysqli_fetch_array($run_task_details);
        if($Data['status'] == 'pending')
        {
            ...
            $insert .= "UPDATE `rider_status` SET  `status` = 'engaged',`latitude`='$latitude',`longitude`='$longitude' WHERE `rider_id` = $rider_id";
        ...
        else
        {
            ...
            [
                "status"=>true,
                "response_code"=>207,
                "message"=>"task was assigned to someone else!",
