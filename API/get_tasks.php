    ...
    if($stats == 'completed')
    {
        ...
        if($rowfortask > 0)
        {
            ...
            while($ar1 = mysqli_fetch_array($runfortask))
            {
                ...
                $sqlforriderloc = "SELECT `id`, `rider_id`, `status`, `tracking`, `latitude`, `longitude` FROM `rider_status` WHERE `rider_id` = '$rider_id'";
                $runsqlforriderloc = mysqli_query($conn,$sqlforriderloc);
                $riderloc = mysqli_fetch_array($runsqlforriderloc);
                $rider_lat = $riderloc['latitude'];
                $rider_lon = $riderloc['longitude'];
