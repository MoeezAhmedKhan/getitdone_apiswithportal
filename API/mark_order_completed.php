...
// UPDATE `task` SET `status`='completed' WHERE `id`='$task_id'
    ...
    // ADD revenue of rider/company
    $sql_insert_revenue = "INSERT INTO `revenue`(`rider_id`, `amount`) VALUES('$rider_id','$grand_total')";
    $exec_insert_revenue = mysqli_query($conn,$sql_insert_revenue);
    ...
    if($execsqlx){
        ...
        $sel_rider_share = "SELECT `id`, `share_percentage`, `created_at` FROM `rider_share`";
        $exec_rider_share = mysqli_query($conn,$sel_rider_share);
        $wecex = mysqli_fetch_array($exec_rider_share);
        $share_percentage = $wecex['share_percentage'];
        ...
        $rider_cut = ($share_percentage * $delivery_fee);
    ...
    $exec_transaction_log1 = mysqli_query($conn , $transaction_log1);
        ...
        //  Total earned amount for rider
        
        $sql_rider_cut = "INSERT INTO `rider_earning`(`rider_id`, `amount`) VALUES
                                                        ('$rider_id','$rider_cut')";
        $exec_rider_cut = mysqli_query($conn, $sql_rider_cut);                                                
